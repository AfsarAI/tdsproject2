...
def post_request(url: str, payload: Dict[str, Any], headers: Optional[Dict[str, str]] = None) -> Any:
    ...
    try:
        ...
        # Update url_time for the new URL if not present
        ...
        if not correct:
            ...
            # Logic to decide whether to retry or move on
            # We use cache count or time limit
            if (cur_url and cache[cur_url] >= retry_limit) or delay >= 180: 
                print("Not retrying, moving on to the next question")
                data = {"url": data.get("url", "")} 
            else: # Retry
                print("Retrying..")
                data["url"] = cur_url if cur_url else next_url
                data["message"] = "Retry Again!" 
        
        print("Formatted: \n", json.dumps(data, indent=4), '\n')
        
        # We do NOT update os.environ here anymore.
        # The agent must handle the state update.
